...
| [`render.sh`](render.sh)                 | Regenerates every `renders/*.png` from the model.       |
| [`concept.html`](concept.html)           | Dimensioned side/front/top drawing (open in a browser). |
| `renders/`                               | PNG previews (regenerated by `render.sh`).              |

## Render / preview

Needs [OpenSCAD](https://openscad.org). Open `typoena-case.scad` in the GUI and
flip the `show` variable, regenerate all previews with **`./render.sh`**, or from
the CLI:

```sh
cd hardware/case
./render.sh                 # regenerate every renders/*.png
./render.sh assembled       # just one view
