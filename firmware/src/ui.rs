...
//!
//! [`Panel`] owns the [`Epd`] and the two reused framebuffers, and encapsulates
...
//! caret ([`Panel::caret_or_sleep`]), the periodic panel-longevity full refresh
...
/// The panel and its refresh state. Owns the [`Epd`] and the only two
...
impl<'d> Panel<'d> {
    ...
    ///
    /// Takes the [`Epd`] by value: the caller keeps it for the boot splash and
    ...
    pub fn new(mut epd: Epd<'d>, ed: &mut Editor) -> Result<Self, EspError> {
    ...
    pub fn render_batch(&mut self, ed: &mut Editor, prev_mode: Mode, keys: u32) {
        ...
        let (result, refresh) = if self.force_full {
            (self.epd.display_frame(self.back.bytes()), "FULL")
        } else if additive {
            (self.epd.display_frame_partial_window(self.back.bytes(), y0, y1 - y0 + 1), "windowed")
        } else {
            (self.epd.display_frame_partial_window(self.back.bytes(), 0, epd::HEIGHT), "full-area")
        };
    ...
    pub fn rest_if_due(&mut self, ed: &mut Editor, focus: &FocusTimer, last_activity: Instant) -> bool {
        ...
        let t0 = Instant::now();
        if let Err(e) = self.epd.display_frame(self.back.bytes()) {
    ...
    pub fn show_notice(&mut self, ed: &mut Editor) -> bool {
        ed.draw_into(&mut self.back, true);
        if let Err(e) = self.epd.display_frame_partial_window(self.back.bytes(), 0, epd::HEIGHT) {
    ...
    /// full-area partial (no flash). Otherwise sleep briefly to yield the CPU.
    /// The tail of the idle sequence — always call it last; it never signals
    /// `continue` because there is nothing after it.
    pub fn caret_or_sleep(&mut self, ed: &mut Editor, last_activity: Instant) {
        if ed.mode() == Mode::Insert
            && !self.cursor_shown
            && last_activity.elapsed().as_millis() >= CURSOR_DEBOUNCE_MS
        {
    ...
    pub fn blit_editor_full(&mut self, ed: &mut Editor) {
        ed.draw_into(&mut self.back, true);
        let _ = self.epd.display_frame(self.back.bytes());
    }
    ...
    pub fn blit_full(&mut self, frame: &Frame) {
        let _ = self.epd.display_frame(frame.bytes());
    }
}
