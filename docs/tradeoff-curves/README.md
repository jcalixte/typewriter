...
| [`epd-refresh-latency.md`](epd-refresh-latency.md) | E-ink refresh latency vs rows driven — flat: the LUT sets the tier (~1870 ms full / ~540 ms partial floor), rows only trim SPI (~0.34 ms/row); the corrected cost model behind typing responsiveness and the boot splash→editor swap.                                                                             |
