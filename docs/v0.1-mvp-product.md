...
3. **Push.** I hit `Ctrl-G`. The device ships my latest changes to GitHub
...
[POWER ON, config present]
  ...
  [EDITING] ← keypresses, partial refresh per edit
        ...
        └── Ctrl-G → save buffer; in parallel: commit locally + bring Wi-Fi up
                    (if nothing staged: status "nothing to push", stop)
                    ...
                    t≈5–10 s, status: "✓ pushed abc1234" or "push failed: <reason>"
...
- **Side panel** (right, ~150 px / ~20 cols, full height): all metadata lives
  ...
  **push state**. `Ctrl-S` briefly flashes "saved HH:MM";
  ...
  (or `push failed: <reason>`, with the local commit preserved either way).
...
| Wi-Fi won't connect                                    | panel Wi-Fi field shows `✗`; editing still works; `Ctrl-G`'s push-state field shows "no network"                                                                                                            |
| `Ctrl-G` push fails (auth, network, merge conflict)    | panel push-state field shows the reason; local commit is preserved; user can retry — non-fast-forward auto-recovers via pull-then-push and is not surfaced as a failure unless the pull itself conflicts    |
...
[qfd-budget.md §6](qfd-budget.md#6-critical-performance-budget); that's the
...
- [ ] `Ctrl-G` produces a commit visible on github.com within 30 s on a
      ...
      has moved on since the last push).
