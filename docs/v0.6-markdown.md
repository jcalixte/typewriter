...
**Status: DELIVERED 2026-07-12** ([macroplan](macroplan.md)) — render
affordances landed early; the snippet engine (tab-stop core, inline
Tab-expansion, hint-on-pause, `$` palette) and the `>` command-palette
generalisation shipped in core (187 editor tests); and slice 5, the
**firmware boot-read + `just init` catalog** (the on-device gate), is done —
an empty snippets file parses as zero snippets, not an error, since
2026-07-13. Snippets are net-new scope, added 2026-07-08; reshaped
...
- **`>`** → _act on the editor_ — the command palette, a real action registry (the
  ...
  **toggle** flips and the list **stays open**; a **one-shot** (`format`,
  `push` — the latter shares `run_push` with `:sync`) runs and **closes**; a
