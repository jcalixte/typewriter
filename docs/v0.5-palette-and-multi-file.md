...
**Status: DELIVERED 2026-07-12** — all four slices confirmed on-device,
firmware 0.5.0 ([macroplan](macroplan.md)). Amended since delivery, in this
spec: the 2026-07-13 palette amendment (recursive walk of `/sd/repo` +
`/sd/local`, `PALETTE_MIN_QUERY = 2`) and the verbatim trailing-newline model.
Two command names in the body are historical: `:e` was retired in v0.6
(palette + `:enew` replace it) and `:sync` has been `:gp` since 2026-07-14.
The design narrative below records v0.5 as built. The single-file `Effect` return became a
drained **effect queue** (`Save{path,contents}` / `Load{path}` / `Push` /
...
(`deleted repo/notes.md - :sync to push`). Deferred to later: greying the
Push affordance for a Local buffer, and the multi-file push count. 123
...
true via a _device_-authored push (`3c79f38`), proving toggle → `SavePrefs` →
...
- [x] Delete a file — **core done (host-tested) 2026-07-12;** `:delete` unlinks
      ...
      the next `:sync` Push's staged set. **Spike 14 (on-device) found the
...
- [~] `Ctrl-G` is disabled / hidden when the current buffer is local-scope —
  **`:sync` / Push is blocked in-core for a Local buffer** (posts "Push
  unavailable (Local)"); the side-panel affordance that hides/greys the
  gesture is the remaining half.
- [ ] The side panel briefly shows file count on `Ctrl-G` when the push bundles
      more than one dirty Tracked file (e.g. `"pushing 3 files: abc1234"`),
...
- [x] **Preferences file** `/sd/repo/.typoena.toml` — a git-tracked,
  ...
  - [ ] `auto_sync` (duration string, default `"10m"`; `"0"` / omitted
        ...
        **opportunistic, rate-limited** Push. Push when already awake + dirty
