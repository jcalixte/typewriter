...
- [x] `:gl` — pull: fetch + **fast-forward only**, refuse on divergence and
      ...
      tracked with the push gap below). How it works:
  - The git thread takes a `GitRequest::Pull` on the same channel as push
    ...
    **ls-refs first**: the ref advertisement alone answers "anything new?", so
    ...
    refused — no merge on the device. Tracking-ref updates keep push's
  ...
  - UI gate: `:gl` is refused while the dirty journal is non-empty
    ("unsynced changes - :sync first", also logged) — an unpushed save
  ...
  - After a `Pulled` outcome the UI drops clean parked buffers (next switch
    ...
    `ahead - :sync to push` / `diverged - resolve on a computer` /
    `pull: <reason>`.
  - **Superseded (v0.9):** divergence is no longer refused — `:gl` now rebases
    ...
    `:gp` to push. The `diverged - resolve on a computer` snackbar is
    replaced by `rebased <oid> - :gp to push`. See
...
- [~] Close the push gap: 19.8 s at the kaizen close — the splice's 5.0 s
  ...
  run 1. **Warm clean push measured 2026-07-14 (run 4, as `:gp` after
