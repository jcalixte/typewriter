...
| `Ctrl-S`        | save                                                                             |
| `Ctrl-G`        | push (save → stage → commit → push, with pull-and-retry on remote divergence) |
...
in [qfd-house-1.md §3](qfd-house-1.md#3-house-of-quality--whats--hows).
...
- **Off by default.** The radio is powered down (`esp_wifi_stop`) except
  during a Push and a short grace window after. This is the load-bearing
  battery decision: always-on station mode burns ~410 mAh/day just for the
  radio; on-demand is closer to ~25 mAh/day at 10 Pushes/day. See
  [qfd-budget.md H13](qfd-budget.md#6-critical-performance-budget).
...
- **Grace window** of ~90 s after a Push completes. A `Ctrl-G` inside
  the window reuses the connection (no second association cost). If no
  Push fires before the window expires, the radio tears down.
- **No reconnect-backoff state machine.** There is nothing to reconnect to
  when the user isn't trying to Push. Single-shot try-on-demand replaces
  the exponential-backoff approach.
- **SNTP** runs on the first successful bring-up of a power session. The
  result is cached for up to 24 h; subsequent Pushes use the cached time
...
[qfd-budget.md §6](qfd-budget.md#6-critical-performance-budget).
...
- Operations needed in v0.1 (the [`gct` shell function](../CONTEXT.md#user-facing-actions) is the reference):
  ...
  - short-circuit return if nothing is staged — status: "nothing to push"
  ...
  - on push failure: `git pull --no-edit` (merge), then retry the push once.
    Only surface failure to the side panel's push-state field if the pull
...
- The whole sequence is atomic from the user's view — see
  [`CONTEXT.md` → Push](../CONTEXT.md#user-facing-actions).
...
export TW_REMOTE_URL=... TW_GH_USER=... TW_TOKEN=...
...
[qfd-tradeoffs.md "Conflicts left explicitly unresolved by v0.1"](qfd-tradeoffs.md#conflicts-left-explicitly-unresolved-by-v01).
